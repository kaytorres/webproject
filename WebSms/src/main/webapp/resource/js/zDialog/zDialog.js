 ...
 * 最后修正：2009-12-18
 **/
var IMAGESPATH = url("resource/js/zDialog/images/"); //图片路径配置
...
Dialog.prototype.create = function () {
    ...
    var DialogDivWidth = this.Width + 10 + 10;
    var DialogDivHeight = this.Height + 30 + 13 + (this.ShowButtonRow ? 40 : 0) + (this.ShowMessageRow ? 50 : 0);

    if (DialogDivWidth > bd.clientWidth) this.Width = Math.round(bd.clientWidth - 20);
    if (DialogDivHeight > bd.clientHeight) this.Height = Math.round(bd.clientHeight - 43 - (this.ShowButtonRow ? 40 : 0) - (this.ShowMessageRow ? 50 : 0));

    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 20) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="10" height="30" style="background-image: url(' + IMAGESPATH + 'Dialog-TopLeft.png) !important;background: url(' + IMAGESPATH + 'Dialog-TopLeft.png) no-repeat 0 0;"><div style="width: 10px;"></div></td>\
      <td height="30" style="background-image:url(' + IMAGESPATH + 'Dialog-Top.png) !important;background: url(' + IMAGESPATH + 'Dialog-Top.png) repeat-x top;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><span style="line-height: 20px;" id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'Dialog-CloseButton.png)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'Dialog-CloseButton.png)\'" style="margin: 2px 0px 0px 3px; position: relative; cursor: pointer; float: right; height: 21px; width: 21px; background-image: url(' + IMAGESPATH + 'Dialog-CloseButton.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
      <td width="10" height="30" style="background-image: url(' + IMAGESPATH + 'Dialog-TopRight.png) !important;background: url(' + IMAGESPATH + 'Dialog-TopRight.png) no-repeat right 0;"><div style="width: 10px;"><a id="_forTab_' + this.ID + '" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
      <td width="10" style="background-image: url(' + IMAGESPATH + 'Dialog-Left.png) !important;background: url(' + IMAGESPATH + 'Dialog-Left.png) repeat-y left;"></td>\
       <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          <tr id="_MessageRow_' + this.ID + '" style="' + (this.ShowMessageRow ? "" : "display:none") + '">\
            ...
            <td width="10" style="background-image: url(' + IMAGESPATH + 'Dialog-Right.png) !important;background: url(' + IMAGESPATH + 'Dialog-Right.png) repeat-y left;"></td>\
   			 </tr>\
          <tr>\
	...
	' + (function (obj) {
      ...
      <td width="4" style="background-image: url(' + IMAGESPATH + 'Dialog-Right.png) !important;background: url(' + IMAGESPATH + 'Dialog-Right.png) repeat-y right;"></td>\
    </tr>\
    <tr>\
      <td width="4" height="10" style="background-image: url(' + IMAGESPATH + 'Dialog-BottomLeft.png) !important;background: url(' + IMAGESPATH + 'Dialog-BottomLeft.png) no-repeat 0 bottom;"></td>\
      <td style="background-image: url(' + IMAGESPATH + 'Dialog-Bottom.png) !important;background: url(' + IMAGESPATH + 'Dialog-Bottom.png) repeat-x bottom;"></td>\
      <td width="4" height="10" style="background-image: url(' + IMAGESPATH + 'Dialog-BottomRight.png) !important;background: url(' + IMAGESPATH + 'Dialog-BottomRight.png) no-repeat right bottom;"><a onfocus=\'$id("_forTab_' + this.ID + '").focus();\' href="#;"></a></td>\
    </tr>\
  </table>\
</div>\
'
    ...
    if (this.URL) {
        ...
        innerFrameOnload = function () {
            try {
                ...
                if (console && console.log) console.log(js_zD1)
            }
...
Dialog.prototype.setSize = function (w, h) {
    if (w && +w > 20) {
        this.Width = +w;
        topWin.$id("_DialogTable_" + this.ID).width = this.Width + 20;
...
Dialog.prototype.autoClose = function () {
    ...
    this.AutoClose -= 1;
    topWin.$id("_Title_" + this.ID).innerHTML = this.AutoClose + js_zD2;
    if (this.AutoClose <= 0) {
        this.close();
    } else {
        var self = this;
...
//    diag.cancelButton.value = js_zD_confirm;
